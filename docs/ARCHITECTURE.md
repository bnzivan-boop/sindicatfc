...
| `friends` | работает | симметричная дружба через заявки (PENDING/ACCEPTED/DECLINED/BLOCKED), рекомендации (общие старты, одноклубники, общие друзья, дисциплина/город), лимиты; влияет на видимость «друзьям», ленту «подписки», приглашения на выезд |
