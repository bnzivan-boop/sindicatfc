...
import { Avatar, DetailTopBar, fontFamily, fontSemi, Page, Round, SectionHead, Surface, T } from '../../../src/components/ui';
  ...
  return (
    <Page>
      ...
      <Surface radius={18} style={{ gap: 8 }}>
        ...
        <Text style={[fontSemi, { fontSize: 16, letterSpacing: -0.4, color: colors.text }]}>{post.title}</Text>
        <T size={11} style={{ lineHeight: 16 }}>{post.text}</T>
        <Pressable onPress={() => (me.data ? like.mutate(post.id) : router.push('/onboarding'))} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, alignSelf: 'flex-start', backgroundColor: post.likedByMe ? `${colors.green}1f` : colors.surface2, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 6 }}>
      ...
      {me.data ? (
        <View style={{ flexDirection: 'row', gap: 7, marginTop: 14 }}>
          <TextInput value={text} onChangeText={setText} onSubmitEditing={send} placeholder="ответить в теме…" placeholderTextColor={colors.muted} style={[fontFamily, { flex: 1, backgroundColor: colors.surface, color: colors.text, borderRadius: 14, paddingHorizontal: 12, paddingVertical: 10, fontSize: 12 }]} />
