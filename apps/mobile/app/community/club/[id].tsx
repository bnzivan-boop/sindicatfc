...
import { Avatar, DeepCard, DetailTopBar, EventTag, fontFamily, Page, Round, SectionHead, Stats, Surface, T, Track } from '../../../src/components/ui';
    ...
    <Page>
      ...
      <DeepCard style={{ borderRadius: 22, padding: 16, marginBottom: 12 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 14, marginBottom: 6 }}>
          ...
          <Text style={[fontFamily, { fontSize: 14, fontWeight: '500', color: colors.white }]}>{c.points.toLocaleString('ru-RU')} очков</Text>
        </View>
      ...
      <View style={{ flexDirection: 'row', gap: 7 }}>
        ...
        <Pressable onPress={() => router.push('/community/messages')} style={{ flex: 1, borderWidth: 1, borderColor: colors.line, borderRadius: 11, padding: 11, alignItems: 'center' }}><T size={9}>чат клуба</T></Pressable>
      </View>
        ...
        <>
          ...
          <View style={{ gap: 6 }}>{c.applicants.map((a) => (
            <Surface key={a.userId} radius={14} style={{ padding: 10, flexDirection: 'row', alignItems: 'center', gap: 9 }}>
              ...
              <Pressable onPress={() => decide.mutate({ userId: a.userId, accept: true })} style={{ backgroundColor: colors.lime, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}><T size={9} color={colors.onLime}>принять</T></Pressable>
              <Pressable onPress={() => decide.mutate({ userId: a.userId, accept: false })} style={{ borderWidth: 1, borderColor: colors.line, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}><T size={9}>отказать</T></Pressable>
            </Surface>
          ))}</View>
        </>
      )}
        ...
        <>
          ...
          <View style={{ gap: 7 }}>{c.slots.map((s, i) => (
            <Surface key={s.id} radius={14} style={{ padding: 11, flexDirection: 'row', alignItems: 'center', gap: 9 }}>
              ...
              {!c.myRole && !c.isCaptain && <Pressable onPress={() => (me.data ? apply.mutate() : router.push('/onboarding'))} style={{ backgroundColor: colors.text, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}><T size={8} color={colors.bg}>откликнуться</T></Pressable>}
            </Surface>
          ))}</View>
        </>
      )}
