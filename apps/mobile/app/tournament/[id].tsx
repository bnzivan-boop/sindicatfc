...
import { DeepCard, DetailSection, DetailTopBar, fontFamily, InfoList, InfoRow, LimeButton, Page, Round, RuleBlock, T, Timeline } from '../../src/components/ui';
    ...
    <Page>
      ...
      <DeepCard minHeight={225}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <View style={{ backgroundColor: t.status === 'FINALIZED' ? whiteAlpha(15) : colors.lime, borderRadius: 999, paddingHorizontal: 9, paddingVertical: 6 }}>
        ...
        <Text style={[fontFamily, { fontSize: 34, lineHeight: 33, letterSpacing: -1.9, fontWeight: '500', color: colors.white, marginTop: 34, marginBottom: 12, maxWidth: 250 }]}>{t.title}</Text>
        ...
        <View style={{ position: 'absolute', right: 18, bottom: 18, alignItems: 'flex-end' }}>
          <Text style={[fontFamily, { fontSize: 31, lineHeight: 32, fontWeight: '500', color: colors.white }]}>{start.getDate()}</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginTop: 10, marginBottom: 18 }}>
        <Pressable onPress={() => router.push({ pathname: '/tournament/[id]/participants', params: { id } })} style={{ flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.green, borderRadius: 15, paddingVertical: 11, paddingHorizontal: 9 }}>
          <Text style={[fontFamily, { fontSize: 9, color: colors.muted, marginBottom: 4 }]}>участники</Text>
          <Text style={[fontFamily, { fontSize: 12, fontWeight: '500', color: colors.text }]}>{t.registeredCount} / {t.capacity} <Text style={{ color: colors.green }}>›</Text></Text>
        </Pressable>
        <View style={{ flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 15, paddingVertical: 11, paddingHorizontal: 9 }}>
          <Text style={[fontFamily, { fontSize: 9, color: colors.muted, marginBottom: 4 }]}>длительность</Text>
          <Text style={[fontFamily, { fontSize: 12, fontWeight: '500', color: colors.text }]}>{hours(t.startsAt, t.endsAt)}</Text>
        </View>
        <View style={{ flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, borderRadius: 15, paddingVertical: 11, paddingHorizontal: 9 }}>
          <Text style={[fontFamily, { fontSize: 9, color: colors.muted, marginBottom: 4 }]}>взнос</Text>
          <Text style={[fontFamily, { fontSize: 12, fontWeight: '500', color: colors.text }]}>{t.entryFee ? money(t.entryFee.amountMinor) : 'бесплатно'}</Text>
        </View>
      </View>

      {myReg ? (
        <Pressable onPress={() => router.push({ pathname: '/registration/[id]', params: { id: myReg.id } })} style={{ marginBottom: 8 }}>
          <View style={{ backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.green, borderRadius: 15, padding: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
